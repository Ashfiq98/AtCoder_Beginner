...
int main()
{
    ...
    {
        cin>>arr[i]>>brr[i];
        total_time +=(arr[i]/brr[i]);
    }
    ld meet_time = total_time/2.0;
    ld rest_time = meet_time;
    ld dist = 0;
    for(ll i=0;i<n;i++)
    {
        ld present_time = arr[i]/brr[i];
        if(rest_time>present_time)
           {
               dist+=arr[i];
               rest_time-=present_time;
           }
        else
        {
            dist+=brr[i]*rest_time;
            break;
        }
    }
